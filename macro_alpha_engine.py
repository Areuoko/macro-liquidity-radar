...
فاز ۶ (اختیاری، خارج از رودمپِ اصلی) — تفسیرِ تحلیلی با Gemini:
 29) یک بخشِ کاملاً اختیاریِ «۶. تفسیر تحلیلی» به گزارش اضافه شد: نسخه‌ی
     ...
     پس از دیدنِ خروجیِ واقعی، systemInstruction دوباره سفت شد: (الف) باید
     با برچسب‌های رسمیِ محاسبه‌شده (market_phase/stress/rotation) هم‌خوان
     باشد، نه روایتِ مستقلِ خودش را از زیرمجموعه‌ای از متریک‌ها بسازد؛
     (ب) برای سطحِ خامِ COT حق ندارد صفتِ «شدید/سنگین» بدهد چون هیچ
     context تاریخی برای قضاوتِ «شدت» در اختیارش نیست — فقط تغییرِ هفتگی.
...
async def fetch_gemini_interpretation(client: httpx.AsyncClient, metrics_summary: str) -> Optional[str]:
    ...
    system_instruction = (
        ...
        "به فارسی، در حداکثر دو پاراگراف کوتاه، رابطه و تناقض‌های واقعی میانِ "
        "متریک‌های زیر را توضیح بده.\n\n"
        "قوانینِ مهم:\n"
        "۱) بخشِ ۵ (وضعیتِ فازِ بازار، شاخصِ استرس، امتیازِ rotation) نتیجه‌ی "
        "محاسبه‌ی رسمیِ خودِ موتور است — روایتِ تو باید با همین برچسب‌ها هم‌خوان "
        "باشد، نه در تضادِ با آن‌ها. مثلاً اگر برچسبِ rotation، NEUTRAL است، "
        "نگو «چرخش به‌سمتِ تدافعی» فقط چون یکی از سه نسبتِ فاز ۳ افت کرده؛ هر "
        "سه نسبت را با هم و در تعادل با برچسبِ رسمی در نظر بگیر، نه فقط "
        "بزرگ‌ترینِ آن‌ها.\n"
        "۲) برای پوزیشنِ COT، فقط دربارهٔ تغییرِ هفتگی (که داده شده) نظر بده، "
        "نه سطحِ خامِ پوزیشن؛ صفت‌هایی مثل «سنگین»، «شدید» یا «تاریخی» را برای "
        "سطحِ خام به‌کار نبر، چون هیچ میانگینِ تاریخی یا بازه‌ی نرمالی در "
        "اختیارت نیست تا «شدت» را نسبت به آن بسنجی — حدسِ بی‌مبنا نزن.\n"
        "۳) اگر داده‌ای برای نتیجه‌گیریِ قطعی کافی نیست، همین را صریح بگو.\n\n"
        "عمیق و مشخص باش، نه کلی‌گویی. در پایان، با یک جمله‌ی کوتاهِ صریح "
        "یادآوری کن که این صرفاً تفسیر است، نه توصیه‌ی مالی یا سیگنالِ معاملاتی."
    )
    payload = {
